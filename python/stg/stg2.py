...
from .models import STGClassificationModel, STGRegressionModel, MLPClassificationModel, MLPRegressionModel, STGCoxModel, \
    ...
    LSPINClassificationModel, LSPINClassificationModel2
...
class STG2(object):
    def __init__(self, model, device, input_dim=784, output_dim=10, hidden_dims=[400, 200],
        ...
        self._model = self.build_model(model, input_dim, output_dim, hidden_dims, activation, sigma, lam,
                                       task_type, feature_selection)
        # self._model.apply(self.init_weights)
        self._model = self._model.to(device)
        self._optimizer = get_optimizer(optimizer, self._model, lr=learning_rate,
    ...
    def build_model(self, model, input_dim, output_dim, hidden_dims, activation, sigma, lam, task_type,
                    feature_selection):
        if task_type == 'classification':
            ...
            if feature_selection:
                if 'gating_net_hidden_dims' in self.extra_args:
                    return LSPINClassificationModel2(model, input_dim, output_dim, hidden_dims,
